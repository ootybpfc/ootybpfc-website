  ...
  return (<AppShell><RequireAuth user={user} isLoading={isLoading}>
    {!isAdmin ? <p className="py-20 text-center text-sm text-muted-foreground" data-testid="admin-forbidden">This console is limited to club admins.</p> : (<>
      ...
      <div className="mt-6 grid gap-3 grid-cols-2 lg:grid-cols-4">
        <Card className="border-border"><CardContent className="p-5"><div className="text-crimson-bright"><Users className="size-4" /></div><p className="mt-3 font-mono text-3xl font-bold" data-testid="admin-kpi-players">{stats.data?.players ?? 0}</p><p className="mt-1 text-[11px] uppercase tracking-wider text-muted-foreground">Players</p></CardContent></Card>
        <Card className="border-border"><CardContent className="p-5"><div className="text-crimson-bright"><ShieldCheck className="size-4" /></div><p className="mt-3 font-mono text-3xl font-bold" data-testid="admin-kpi-coaches">{stats.data?.coaches ?? 0}</p><p className="mt-1 text-[11px] uppercase tracking-wider text-muted-foreground">Coaches</p></CardContent></Card>
        <Card className="border-border"><CardContent className="p-5"><div className="text-crimson-bright"><Image className="size-4" /></div><p className="mt-3 font-mono text-3xl font-bold" data-testid="admin-kpi-media">{stats.data?.pending_media ?? 0}</p><p className="mt-1 text-[11px] uppercase tracking-wider text-muted-foreground">Media pending</p></CardContent></Card>
        <Card className="border-border"><CardContent className="p-5"><div className="text-crimson-bright"><AlertTriangle className="size-4" /></div><p className="mt-3 font-mono text-3xl font-bold" data-testid="admin-kpi-checks">{stats.data?.background_checks_due ?? 0}</p><p className="mt-1 text-[11px] uppercase tracking-wider text-muted-foreground">Checks due</p></CardContent></Card>
      </div>
      {revenue.length > 0 ? <Card className="mt-3 border-border"><CardContent className="flex flex-wrap gap-8 p-5">{revenue.map(([cur, amount]) => <div key={cur}><p className="text-[11px] uppercase tracking-wider text-muted-foreground">Collected ({cur})</p><p className="font-mono text-2xl font-bold text-crimson-bright">${amount.toFixed(2)}</p></div>)}</CardContent></Card> : null}
