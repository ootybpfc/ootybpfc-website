...
export default function CoachInvitePage() {
  ...
  return (<AppShell><div className="grid gap-8 py-6 md:grid-cols-2 md:gap-14"><div className="animate-pearl-rise"><Badge className="border-gold/40 bg-gold-soft text-gold">You've been invited</Badge><h1 className="mt-4 font-heading text-3xl font-black uppercase tracking-tighter md:text-5xl">{data ? `Hi ${data.name.split(" ")[0]}` : "Coach invite"}</h1>{data ? (<Card className="mt-6 border-border"><CardContent className="p-5"><p className="flex items-center gap-2 font-heading text-sm font-semibold"><ShieldCheck className="size-4 text-gold" /> Your assignment</p><ul className="mt-3 space-y-1.5">{data.org_names.map((n) => (<li key={n} className="flex items-center gap-2 text-sm"><MapPin className="size-3.5 shrink-0 text-gold" />{n}</li>))}</ul></CardContent></Card>) : null}</div><Card className="self-start border-border"><CardContent className="p-6"><form className="space-y-5" onSubmit={(e) => { e.preventDefault(); accept.mutate(); }} data-testid="invite-form"><p className="font-heading text-sm font-semibold">Finish your profile</p><div className="space-y-2"><Label htmlFor="invite-phone">Phone</Label><Input id="invite-phone" type="tel" className="h-12" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} /></div><div className="space-y-2"><Label htmlFor="invite-exp">Experience</Label><Textarea id="invite-exp" value={form.experience} onChange={(e) => setForm({ ...form, experience: e.target.value })} /></div><Button type="submit" className="h-12 w-full bg-crimson text-white hover:bg-crimson-bright" disabled={accept.isPending || !data} data-testid="invite-accept-button">{accept.isPending ? "Setting up…" : "Accept and activate my account"}</Button></form></CardContent></Card></div></AppShell>);
}
