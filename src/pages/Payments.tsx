...
export function PaymentsPage() {
  ...
  return (<AppShell><RequireAuth user={user} isLoading={isLoading}>
    ...
    {user?.role !== "coach" ? (<Card className="mt-6 border-crimson/30" data-testid="checkout-panel"><CardContent className="space-y-4 p-5">
...
export function PaymentSuccessPage() {
  ...
  return (<AppShell><div className="py-20 text-center" data-testid="payment-success-page">{state === "paid" ? <CheckCircle2 className="mx-auto size-10 text-pitch" /> : <CreditCard className="mx-auto size-10 text-crimson-bright" />}<h1 className="mt-4 font-heading text-3xl font-black uppercase tracking-tighter">{state === "paid" ? "Payment confirmed" : state === "polling" ? "Confirming payment…" : "Still processing"}</h1><Link to="/payments" className="mt-8 inline-flex h-12 items-center rounded-md bg-crimson px-6 text-sm font-semibold text-white">Back to payments</Link></div></AppShell>);
}
